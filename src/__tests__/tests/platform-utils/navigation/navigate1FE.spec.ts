...
// TODO[1fe]: Remove skip and Test again
test.skip('navigate within widget with URL updates should only persist runtime config param', async ({
  page,
}) => {
  await page.goto(
    `http://localhost:3001/app1/utils?runtime_config_overrides={}&test=test`,
  );
...
test('navigate from plugin to plugin with URL updates should only persist runtime config param', async ({
  page,
}) => {
  await page.goto(
    `http://localhost:3001/app1/utils?runtime_config_overrides={}&test=test`,
