...
test('return correct route with query param', async ({ page }) => {
  ...
  page.on('dialog', async (dialog) => {
    expect(dialog.message().startsWith('/utils?test=test')).toBeTruthy();
...
test('return correct route without query param', async ({ page }) => {
  ...
  page.on('dialog', async (dialog) => {
    expect(dialog.message().startsWith('/utils')).toBeTruthy();

    await dialog.dismiss();
  });

  await page.click(
    'button[data-qa="utils.navigation.getRoute.excludeParams.btn"]',
  );
});
