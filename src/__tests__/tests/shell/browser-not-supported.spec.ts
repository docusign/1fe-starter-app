...
const checkForVisibleUnsupportedBrowserPage = async (page: Page, isSupported = false) => {
...
test.describe('test outdated user agent (chrome)', () => {
    test.use({ userAgent: 'Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/40.0.2214.93 Safari/537.36' });
...
test.describe('future latest agent (chrome)', () => {
    ...
    test(
        'should NOT show "unsupported browser" page @e2e',
        async ({ page }) => {
