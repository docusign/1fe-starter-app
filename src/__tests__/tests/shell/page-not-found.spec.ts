...
test(
  'the app redirects to HTTP_404_NOT_FOUND_URL on unmatched route',
  async ({ page }) => {
    await page.goto('http://localhost:3001/not-valid');

    const notFoundHeader = page.locator('h1[data-qa="shell.notFound.page.header"]');
...
test(
  'the app redirects to HTTP_404_NOT_FOUND_URL on unmatched route with valid query params',
  async ({ page }) => {
...
test(
  'If HTTP_404_NOT_FOUND_URL is not send, can use the go back button to go back a page',
  async ({ page }) => {
