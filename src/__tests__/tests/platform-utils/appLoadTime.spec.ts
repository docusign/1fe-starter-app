...
test('Should successfully Mark Start and Mark End for generic-child-widget', async ({ page }) => {
    ...
    await expect(resultElement).not.toContainText(
      '@1fe/starter-kit2-start',
    );
    await expect(resultElement).not.toContainText(
      '@1fe/starter-kit2-end',
    );
...
test('Should mark and measure custom events', async ({ page }) => {
  ...
  expect(/\d+(\.\d+)?/.test((await resultElement.innerText()) || '')).toBeTruthy();
});